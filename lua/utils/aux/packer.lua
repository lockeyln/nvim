...
function aux_packer.entry(plugins)
    Packer_bootstrap = (function()
        local packer_install_path = fns.path.join(vim.fn.stdpath("data"), "site/pack/packer/start/packer.nvim")
        ...
        function(use)
            for plugin_kind_name, plugin_kind_tbl in pairs(plugins) do
                for _, plugin_opts in ipairs(plugin_kind_tbl) do
                    ...
                    local require_path = fns.path.join("configs", plugin_kind_name, require_name)
                    ...
                    if ok then
                        if module._viml then
                            ...
                            plugin_opts.config = string.format(
                                [[
                                require('utils.fns').safe_require(require('%s'))
                                require('%s').load()
                                require('%s').after()
                                ]],
                                require_path,
